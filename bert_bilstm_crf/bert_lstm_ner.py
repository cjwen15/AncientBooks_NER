...
if os.name == 'nt':
    bert_path = "..\bert_bilstm_crf\chinese_roberta_wwm_ext_L-12_H-768_A-12"
    root_path = r'..\bert_bilstm_crf'
else:
    bert_path = '../bert_bilstm_crf/chinese_roberta_wwm_ext_L-12_H-768_A-12'
    root_path = '../bert_bilstm_crf'
...
flags.DEFINE_string(
    "task_name", 'ancientbooksner', "The name of the task to train."
)
...
flags.DEFINE_string("vocab_file", os.path.join(bert_path, 'vocab5.txt'),  # vocab.txt
...
class Ancientbooks_NERProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "0922/train_1-10424_BIO_split100.txt")), "train")

    def get_dev_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "dev_BIO_split100.txt")), "dev")

    def get_test_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "0922/NER-test3_afterFormat_split100.txt")), "test")
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(label_ids, logits, trans):
                ...
                precision = tf_metrics.precision(label_ids, pred_ids, num_labels, [1, 2, 3, 4], weight)  #  pred_ids, [2, 3, 4, 5], weight
                recall = tf_metrics.recall(label_ids, pred_ids, num_labels, [1, 2, 3, 4], weight)  # weight
                f = tf_metrics.f1(label_ids, pred_ids, num_labels, [1, 2, 3, 4], weight)  # weight
...
def main():
    ...
    processors = {
        "ancientbooksner": Ancientbooks_NERProcessor
    }
...
#         raise ValueError("At least one of `do_train` or `do_eval` must be True.")
    ...
    if FLAGS.do_predict:
        ...
        # 原来的路径
        ##############################################################
        # 带标签的测试集在这里被转化成不带标签的token_test.txt,同时加上了[CLS],[SEP],PAD
...
def load_data():
    processer = Ancientbooks_NERProcessor()
